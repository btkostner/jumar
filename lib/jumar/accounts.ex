defmodule Jumar.Accounts do
  ...
  def register_user(attrs) do
    ...
    |> Repo.insert()
    |> EventBusProducer.produce_from_result(:user_created)
  end
  ...
  def update_user_email(user, token) do
    ...
    Repo.transact(fn ->
      with {:ok, query} <- UserToken.verify_change_email_token_query(token, context),
           ...
           :ok <- EventBusProducer.produce(:user_email_updated, user),
           {_count, _result} <-
  ...
  defp update_user_and_delete_all_tokens(changeset) do
    Repo.transact(fn ->
      with {:ok, user} <- Repo.update(changeset) do
        ...
        :ok = EventBusProducer.produce(:user_updated, user)
