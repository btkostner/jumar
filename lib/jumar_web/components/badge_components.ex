defmodule JumarWeb.BadgeComponents do
  @moduledoc """
  ...
  attr :color, :string, values: Map.keys(@colors), default: "zinc"
  ...
  def badge(assigns) do
    assigns =
      assign(assigns, :class, [
        ...
        @colors[assigns.color],
        assigns.class
      ])

    ~H"""
    <span {@rest} class={@class}><%= render_slot(@inner_block) %></span>
    """
  end
  ...
  def badge_button(assigns) do
    ...
    <%= if @rest[:href] || @rest[:navigate] || @rest[:patch] do %>
      <.link {@rest} class={@class}>
        <.touch_target>
          <.badge color={@color}><%= render_slot(@inner_block) %></.badge>
        </.touch_target>
      </.link>
    <% else %>
