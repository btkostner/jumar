defmodule JumarWeb.ButtonComponents do
  @moduledoc """
  You know, those things you click to do just about
  anything in a web application.
  """
  ...
  """
  attr :color, :string, default: "dark/zinc", values: Map.keys(@styles.colors)
  attr :outline, :boolean, default: false
  attr :plain, :boolean, default: false

  attr :class, :string, default: ""
  attr :rest, :global, include: ~w(href navigate patch method name value disabled)
  ...
  def button(%{rest: rest} = assigns) do
    style_classes =
      cond do
        assigns[:outline] -> @styles.outline
        assigns[:plain] -> @styles.plain
        true -> [@styles.solid, @styles.colors[assigns[:color]]]
      end
  ...
  def touch_target(assigns) do
    ~H"""
    <span
      class="size-[max(100%,2.75rem)] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-fine:hidden"
      aria-hidden="true"
    />
